...
2. **MV Data** (`MVs Size` bytes): Motion vector data for inter blocks.
   ...
   - **Profile 0x02**: Begins with a 1-byte mode flag: low bits select the prediction mode (`0x00` = Raw values, `0x01` = Spatial prediction residuals via `median(A, B, C)`, `0x02` = Temporal co-located prediction residuals against the previous coded frame's reconstructed MV array — both sides hold zeros at skip positions so the arrays are index-aligned), bit 7 (`0x80`) selects backward-adaptive history tables for the value models instead of in-stream compressed frequency tables (same acc = acc/2 + cur lockstep as the coefficient streams; state resets at every I-frame). The encoder cost-selects the smallest of raw / spatial / temporal, each with static-vs-history models. Decoders restore reconstructed MVs sequentially to feed subsequent spatial predictions; skipped neighbor blocks use (0,0).
...
> Wave 1): both sides maintain a quarter-resolution L0 reference chain, so
> the `maxLayer=0` output is bit-exact with the encoder's L0 reconstruction.
> This is normative for Profile 0x02 P-frames whenever an L0 reference from
> a preceding I-frame exists; it is not signaled per-stream.
