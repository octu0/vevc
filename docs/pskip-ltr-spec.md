...
- **Zero-Tree Flag Aggregation (treeMap) (2026-08-20):** Profile 0x02 P-frames serialize a `treeMap` bitmap (`treeMapSize` in header) aggregating full-zero trees across all 3 layers (L0, L1, L2) for each 32x32 inter region across Y, Cb, Cr planes. Each plane packs 1 bit per `inter` tree in ascending order (`1` = tree is all-zero across all layers). Trees with bit=1 omit per-block flag signaling (isZero/split) across L0, L1, and L2; decoders infer isZero=true (split=false). Non-treez inter trees continue to write normal per-block flags for non-skip blocks.
...
- **Finding:** the Base8 luma path clears whole blocks whose source-vs-prediction SAD is below a qstep-scaled threshold (`preparePlaneBase8WithSkipMap`), but Cb/Cr had no analog. On miko1 motion clips (frames 600–720) the chroma nonzero-block rate ran 18.7% (Cb) / 22.3% (Cr) vs luma's 7.8%, making P-L0 chroma cost MORE than luma (P-L0 split at miko1 500k: Y 199 kbps / Cb 95 / Cr 153).
- **Fix:** `extractSingleTransformBlocksBase8WithSkipMap(cullSAD:)` culls a chroma block pre-DWT when its raw 8×8 residual SAD (`base8BlockSAD`) is below `scaledSADThreshold(96, step:)` — the same qstep-proportional curve as the luma gate, reaching 512 at the saturated step (real 256). Gated on saturation (`qtC.step >= 2048` Q4 = real 128), so normal-rate streams are untouched; env `VEVC_L0C_CULL` overrides the base SAD (0 disables). All-zero coefficients are always legal, so this is encoder-only and bit-exact by construction.
- **Measured (miko1 1080p60 @500k ABR):** size −10% (1616 → 1455 kbps), PSNR avg −0.16 dB, SSIM avg −0.001 / min +0.0008 (improved); per-plane chroma PSNR −0.23〜−0.26 dB avg (worst frame −2.2 dB @ frame 604). At `-b 2500` the stream is essentially bit-identical to baseline (−573 B / 11.66 MB). Visual check on frames 604/619/700 found no new artifacts; HUD text intact.
- **Rejected alternatives (same investigation):** blanket L0 temporal decimation (`-l0cadence`, kept as an opt-in experimental flag, default off) collapsed worst-frame quality catastrophically — undetected hard cuts crossed by thinned frames output stale scenes for 13+ consecutive frames (PSNR ≈ 8.9 windows); ~86% of miko1 P-frames carry ≥10% inter blocks so uniform temporal thinning fights constant motion. Saturation-ramped qLow/chroma-detail quantization extensions measured −12% size for −1.36 dB PSNR — worse RD than baseline and reverted.

### 9.9. MV Stream Coding (2026-08-22, profile 0x02)

- **Adopted — backward-adaptive value models:** the P-frame MV payload's two rANS value models (dx/dy) can now reuse `acc = acc/2 + cur` history built from previously coded frames instead of writing ~40–85 B of compressed frequency tables per frame (mode flag bit `0x80`; Shannon cross-entropy cost-selects per candidate). miko1 @500k: −1.6% stream size at bit-identical quality metrics.
- **Adopted — temporal co-located prediction (mode `0x02`):** residual against the previous coded frame's MV array, cost-selected alongside raw/spatial. Measured nearly free on miko1 (−0.03%): ME re-searches per frame and jitters between quarter-pel local minima even for consistent motion, so co-located residuals are as large as raw values. Kept because cost-selection makes it free and it targets content where ME is temporally stable.
