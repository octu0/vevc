...
final class QualityDropTests: XCTestCase {
    ...
    func xtestQualityDropOnFrame() async throws {
        ...
        var (sub16, l1yBlocks, l1cbBlocks, l1crBlocks, rel16) = try await preparePlaneLayer16(
            pd: pd, pool: pool, sads: nil, layer: 1, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { rel16() }
        let (b8ReconBytes, b8Recon, _, _, _, relb8_a) = try await encodePlaneBase8(
            pd: sub16, pool: pool, sads: nil, layer: 0, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { relb8_a() }
        ...
        var (sub16_2, l1yBlocks_2, l1cbBlocks_2, l1crBlocks_2, rel16_2) = try await preparePlaneLayer16(
            pd: sub32, pool: pool, sads: nil, layer: 1, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { rel16_2() }
