...
final class QualityDropTests: XCTestCase {
    ...
    func testQualityDropOnFrame() async throws {
        ...
        // 1. Base8
        let codec = Layer0CodecFactory.create(profile: 1)
        let (bytesB8, reconB8, _, _, _, relb8) = try await codec.encode(
            pd: pd, pool: pool, sads: nil as [Int]?, occlusionScores: nil as [Int]?, layer: 0, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { relb8() }
        let (decB8, _, _, _, _, _) = try await codec.decode(r: bytesB8, pool: pool, layer: 0, dx: pd.width, dy: pd.height, isIFrame: true)
        ...
        defer { rel16() }
        let (b8ReconBytes, b8Recon, _, _, _, relb8_a) = try await codec.encode(
            pd: sub16, pool: pool, sads: nil as [Int]?, occlusionScores: nil as [Int]?, layer: 0, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { relb8_a() }
        ...
        let (_, _, _, _, _, relb8_b) = try await codec.encode(
            pd: sub16, pool: pool, sads: nil as [Int]?, occlusionScores: nil as [Int]?, layer: 0, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { relb8_b() }
        let (decB8_sub16, _, _, _, _, _) = try await codec.decode(r: b8ReconBytes, pool: pool, layer: 0, dx: pd.width, dy: pd.height, isIFrame: true)
        ...
        defer { rel16_2() }
        let (_, b8Recon_2, _, _, _, relb8_c) = try await codec.encode(
            pd: sub16_2, pool: pool, sads: nil as [Int]?, occlusionScores: nil as [Int]?, layer: 0, qtY: qtY, qtC: qtC, zeroThreshold: 3)
        defer { relb8_c() }
        ...
        let (reconL1Cb_2, _) = reconstructPlaneLayer16Cb(
            blocks: l1cbBlocks_2, prevImg: baseImg2, width: (sub32.width + 1) / 2, height: (sub32.height + 1) / 2, qt: qtC, pool: pool)
        let (reconL1Cr_2, _) = reconstructPlaneLayer16Cr(
        ...
        let (r1Cb, _) = reconstructPlaneLayer16Cb(
            blocks: l1cbBlocks_2, prevImg: prevImg8, width: (sub32.width + 1) / 2, height: (sub32.height + 1) / 2, qt: qtC, pool: pool)
        let (r1Cr, _) = reconstructPlaneLayer16Cr(
            blocks: l1crBlocks_2, prevImg: prevImg8, width: (sub32.width + 1) / 2, height: (sub32.height + 1) / 2, qt: qtC, pool: pool)
