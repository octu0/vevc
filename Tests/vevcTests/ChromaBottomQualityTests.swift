...
final class ChromaBottomQualityTests: XCTestCase {
    ...
    private func assertChromaBottomQuality(
        ...
        if cbBottomPsnr <= 50.0 && maxPsnrDifference < cbDiff {
            XCTFail(
                "\(frameLabel) Cb上下PSNR差(\(String(format: "%.1f", cbDiff))dB)が\(maxPsnrDifference)dBを超えている (上=\(String(format: "%.1f", cbTopPsnr))dB, 下=\(String(format: "%.1f", cbBottomPsnr))dB) → 下部に集中的なカラーノイズ"
            )
        }
        if crBottomPsnr <= 50.0 && maxPsnrDifference < crDiff {
            XCTFail(
                "\(frameLabel) Cr上下PSNR差(\(String(format: "%.1f", crDiff))dB)が\(maxPsnrDifference)dBを超えている (上=\(String(format: "%.1f", crTopPsnr))dB, 下=\(String(format: "%.1f", cbBottomPsnr))dB) → 下部に集中的なカラーノイズ"
            )
        }
    }
