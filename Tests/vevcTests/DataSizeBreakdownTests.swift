...
struct DataSizeBreakdownTests {
    ...
    @Test func perBlockOverhead4x4() {
        ...
        do {
            ...
            encoder.addPair(run: 0, val: 1, isParentZero: false)
        ...
        do {
            ...
            encoder.addPair(run: 15, val: 1, isParentZero: false) // run=15 to reach (3,3) in 4x4
            encoder.flush()
            let data = encoder.getData()
        ...
        do {
            ...
            for v in vals {
                if v == 0 {
                    ...
                    encoder.addPair(run: run, val: v, isParentZero: false)
                    run = 0
                }
            }
            if run > 0 {
    ...
    @Test func multipleBlocksOverhead() {
        ...
        for count in blockCounts {
            ...
            for _ in 0..<count {
                ...
                encoder.addPair(run: 0, val: 2, isParentZero: false) // val=+2
                encoder.addPair(run: 1, val: 1, isParentZero: false) // skip 1, val=+1
                encoder.addPair(run: 0, val: -1, isParentZero: false) // val=-1
    ...
    @Test func bypassVsRansDataRatio() {
        ...
        for _ in 0..<200 {
            ...
            encoder.encodeBypass(binVal: 1) // lscpX
            encoder.encodeBypass(binVal: 1) // lscpY
            encoder.addPair(run: 0, val: 3, isParentZero: false)
            encoder.addPair(run: 1, val: 1, isParentZero: false)
            encoder.addPair(run: 0, val: -2, isParentZero: false)
    ...
    @Test func estimateFrameDataDistribution() {
        ...
        for _ in 0..<(nonZeroBlocks * 3) {
            ...
            encoder2.addPair(run: 0, val: 2, isParentZero: false)
            encoder2.addPair(run: 2, val: 1, isParentZero: false)
            encoder2.addPair(run: 0, val: -1, isParentZero: false)
