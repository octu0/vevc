...
final class BidirectionalPredictionTests: XCTestCase {
    ...
    func testBidirectionalMVSearchSelectsBetterReference() async throws {
        ...
        let (_, fwdSADs) = await computeMotionVectors(curr: pd1, prev: pd0, prevMVs: nil, pool: pool, roundOffset: 0)
        let (_, bwdSADs) = await computeMotionVectors(curr: pd1, prev: pd2, prevMVs: nil, pool: pool, roundOffset: 0)
