...
private func decodeSinglePlane(
    ...
    try buf.withUnsafeBufferPointer { ptr in
        ...
        for tileInfo in mapTiles {
            ...
            inverseIntraDwt2D(base: block.base, size: tileInfo.size >> dropLevels, stride: block.stride, levels: targetLevels, filter: tileInfo.filter)
