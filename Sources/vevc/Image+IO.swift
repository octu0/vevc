...
public func pngToYCbCr(data: Data) throws -> YCbCrImage {
    ...
    for y in 0..<height {
        for x in 0..<width {
            ...
            let yVal = ((((1 * 19595 * r1) + (1 * 38470 * g1)) + (1 * 7471 * b1)) + (1 * (1 << 15))) >> 16
            let cbVal = ((((((1 * -11059 * r1) - (1 * 21709 * g1)) + (1 * 32768 * b1)) + (1 * (1 << 15))) >> 16) + 128)
            let crVal = ((((((1 * 32768 * r1) - (1 * 27439 * g1)) - (1 * 5329 * b1)) + (1 * (1 << 15))) >> 16) + 128)

            let yIdx = ycbcr.yOffset(x, y)
            ycbcr.yPlane[yIdx+0] = UInt8(clamping: yVal)

            let cOff = ycbcr.cOffset(x, y)
             if cOff < ycbcr.cbPlane.count {
                ycbcr.cbPlane[cOff+0] = UInt8(clamping: cbVal)
                ycbcr.crPlane[cOff+0] = UInt8(clamping: crVal)
            }
        }
    }
...
public func saveImage(img: YCbCrImage, url: URL) throws {
    ...
    for y in 0..<height {
        for x in 0..<width {
            let yScaled = (1 * (Int(img.yPlane[img.yOffset(x, y)]) << 10))
            ...
            let cbDiff = (Int(img.cbPlane[cOff+0]) - 128)
            let crDiff = (Int(img.crPlane[cOff+0]) - 128)

            let r = ((yScaled + (1 * 1436 * crDiff)) >> 10)
            let g = (((yScaled - (1 * 352 * cbDiff)) - (1 * 731 * crDiff)) >> 10)
            let b = ((yScaled + (1 * 1815 * cbDiff)) >> 10)
    ...
    guard let colorSpace = CGColorSpace(name: CGColorSpace.sRGB) else {
        throw NSError(domain: "ImageError", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to create color space for output"])
    }
    guard let context = CGContext(
        data: &rawData,
        width: width,
