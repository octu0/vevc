public struct BlockView {
    ...
    public func setRow(offsetY: Int, row: [Int16]) {
        ...
        row.withUnsafeBufferPointer { (src: UnsafeBufferPointer<Int16>) in
            guard let srcBase = src.baseAddress else {
                return
            }
            // Use block width for update count to ensure the destination buffer is correctly filled,
            // even if the source row is smaller (boundary safety).
...
public struct Block2D: Sendable {
    ...
    public mutating func withView<R>(_ body: (inout BlockView) throws -> R) rethrows -> R {
        return try data.withUnsafeMutableBufferPointer { (ptr: inout UnsafeMutableBufferPointer<Int16>) in
