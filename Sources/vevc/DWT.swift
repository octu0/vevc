...
internal func lift53Scalar(_ buffer: UnsafeMutableBufferPointer<Int16>, count: Int, stride: Int) {
    ...
    for i in 0..<half {
        low[i+0] = buffer[((2 * i) * stride)]
        high[i+0] = buffer[(((2 * i) + 1) * stride)]
    }
    ...
    for i in 0..<half {
        ...
        high[i+0] -= Int16(((l + r) >> 1))
    }

    for i in 0..<half {
        let d = Int32(high[i+0])
        var dp = Int32(high[i+0])
        if 0 <= (i - 1) {
            dp = Int32(high[(i - 1)])
        }
        low[i+0] += Int16((((dp + d) + 2) >> 2))
    }

    for i in 0..<half {
        buffer[(i * stride)] = low[i+0]
        buffer[((half + i) * stride)] = high[i+0]
    }
}
...
internal func invLift53Scalar(_ buffer: UnsafeMutableBufferPointer<Int16>, count: Int, stride: Int) {
    ...
    for i in 0..<half {
        low[i+0] = buffer[(i * stride)]
        high[i+0] = buffer[((half + i) * stride)]
    }
    ...
    for i in 0..<half {
        buffer[((2 * i) * stride)] = low[i+0]
        buffer[(((2 * i) + 1) * stride)] = high[i+0]
    }
}
