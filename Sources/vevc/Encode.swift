...
@inline(__always)
func shouldSplit32(data base: UnsafeMutablePointer<Int16>, skipLL: Bool) -> Bool {
...
@inline(__always)
func shouldSplit16(data base: UnsafeMutablePointer<Int16>, skipLL: Bool) -> Bool {
    var q0 = false, q1 = false, q2 = false, q3 = false
...
@inline(__always)
func isEffectivelyZeroBase4(data base: UnsafeMutablePointer<Int16>, threshold: Int, isPFrame: Bool = false) -> Bool {
...
func encodePlaneSubbands16(blocks: inout [Block2D], zeroThreshold: Int, parentBlocks: [Block2D]?, colCount: Int = 0, rowCount: Int = 0) -> [UInt8] {
    ...
    for i in blocks.indices {
        ...
        if isZero {
            ...
            bwFlags.writeBit(false)
            let forceSplit = shouldSplit16(data: blocks[i].base, skipLL: true)
            if forceSplit {
                bwFlags.writeBit(true)
...
func encodePlaneSubbands8(blocks: inout [Block2D], zeroThreshold: Int) -> [UInt8] {
    ...
    for i in blocks.indices {
        let isZero = isEffectivelyZero8(data: blocks[i].base, threshold: zeroThreshold)
        if isZero {
            bwFlags.writeBit(true)
            bwFlags.writeBit(false)
        } else {
...
func encodePlaneBaseSubbands8(blocks: inout [Block2D], zeroThreshold: Int, isPFrame: Bool = false) -> [UInt8] {
    ...
    for i in blocks.indices {
        let isZero = isEffectivelyZeroBase4(data: blocks[i].base, threshold: zeroThreshold, isPFrame: isPFrame)
        if isZero {
            bwFlags.writeBit(true)
            bwFlags.writeBit(false)
...
func encodePlaneBaseSubbands32(blocks: inout [Block2D], zeroThreshold: Int) -> [UInt8] {
    ...
    for i in blocks.indices {
        ...
        if isZero {
            ...
            bwFlags.writeBit(false)
            let forceSplit = shouldSplit32(data: blocks[i].base, skipLL: false)
            if forceSplit {
                bwFlags.writeBit(true)
...
func encodeSubbands16WithoutParent<M: EntropyModelProvider>(
    ...
    case .split4(let tl, let tr, let bl, let br):
        if tl {
            ...
            let lh = BlockView(base: subs.lh.base, width: 4, height: 4, stride: 16)
            let hh = BlockView(base: subs.hh.base, width: 4, height: 4, stride: 16)
            blockEncode4(encoder: &encoder, block: hl, parentBlock: nil)
            blockEncode4(encoder: &encoder, block: lh, parentBlock: nil)
            blockEncode4(encoder: &encoder, block: hh, parentBlock: nil)
        }
        if tr {
            let hl = BlockView(base: subs.hl.base.advanced(by: 4), width: 4, height: 4, stride: 16)
            let lh = BlockView(base: subs.lh.base.advanced(by: 4), width: 4, height: 4, stride: 16)
